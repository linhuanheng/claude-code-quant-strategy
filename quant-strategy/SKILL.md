...
Claude Code 将此文件自动加载为项目上下文。必须包含以下全部内容，不得省略任何章节：
...
**职责**：审查代码中是否引入了未来信息（7 类前视偏差）。只读代码、报告违规，不修改代码。
...
| 用户修改了涉及数据或计算的代码后 | 变更文件 + 受影响的调用链文件 |
| 用户说"帮我检查有没有未来信息" | 用户指定范围或全部文件 |

**绝对禁止**：在审计通过之前向用户展示任何收益数字、分布统计、显著性结果、净值曲线。
...
2. 实现策略：编辑 `strategy/base.py` 中的 `YourStrategy` 类
