...
Claude Code 将此文件自动加载为项目上下文。必须包含以下内容，确保 Claude 在后续协作中遵守项目规则：
...
### 1. 禁止引入未来信息
在任意时间点 t 的计算只能使用时刻 t 及之前已知的信息：
- 因子标准化：必须使用 rolling/expanding window，禁止全样本均值/标准差
- 分位数分组：每期截面独立计算，或 expanding window 阈值，禁止全样本排序分组
- 财务数据：必须考虑披露延迟（年报 +4 月、中报 +2 月、季报 +1 月），禁止在公告日前使用
- 信号执行：T 日信号必须在 T+1 日或之后执行，禁止 T 日收盘价执行
- ML 模型：训练/验证/测试必须按时序切分（walk-forward），禁止 shuffle 或随机交叉验证
- 因子筛选：IC/ICIR 必须用滚动窗口，禁止全样本筛选后再回测

### 2. 收益分析规范
每次汇报收益分析结果必须包含：
- 完整收益分布：Mean、Median、P5、P25、P40、P75、P95
- OLS t 检验 + Newey-West 自相关稳健 t 检验（滞后阶数：4 × (T/100)^(2/9)）
- 经自相关调整的年化夏普比率（Lo 方法）
- 显著性水平标注：*** p<0.01, ** p<0.05, * p<0.10

### 3. 审计前置
在向用户展示任何收益分析结果之前，必须先调用 look-ahead 审计 Agent 检查代码：
- 审计 Agent 配置文件：`sub-agent/lookahead-auditor.md`
- 审计通过后，才能展示分析结果
- 审计未通过，必须先修复再重审

## 开发流程
1. 策略逻辑写在 `strategy/base.py` 中
2. 配置参数在 `config.py` 中修改
3. 因子定义在 `factors/builder.py` 中添加
4. 运行 `python main.py` 执行完整回测
5. 编辑代码后，再次审计，确认无前视偏差后查看结果

## 开发日志
所有开发改动自动记录在 `DEV_LOG.md` 中，精确到分钟。
日志 Agent 配置文件：`sub-agent/dev-logger.md`
